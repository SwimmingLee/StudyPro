...
export default {
  ...
  methods: {
    async get_stream(stream) {
      ...
      this.share_block.childNodes[0] == "img"
        ? 0
        : (this.share_block.childNodes[0].srcObject = this.local_stream);
    },
    ...
    createImg(i) {
      ...
      t_img.src = i
        ? require("../../assets/images/noru.jpg")
        : require("../../assets/images/now_sharing.jpg");
  ...
  mounted() {
    ...
    this.socket.on("viewsharestart", sharing_user_id => {
      ...
      for (let i of this.share_block.childNodes)
        this.share_block.removeChild(i);
